...
public class CommandHandler implements TabExecutor {
    ...
    private boolean placeCampBanner(Location location, Player player) {
        ...
        // Set banner rotation based on player facing direction
        if (banner.getBlockData() instanceof org.bukkit.block.data.Rotatable rotatable) {
            rotatable.setRotation(getBannerRotationFromPlayer(player));
        ...
        // Try to place camp structure
        final var campStructure = new CampStructure(plugin, player.getUniqueId());
        if (campStructure.placeCampStructure(location, player)) {
            campBannerMap.addCampStructure(location, campStructure);
        }
    ...
    private void showWallet(Player player) {
        ...
        player.sendMessage(
                Component.text("• Right-click inactive waypoints to spend tokens (1 per click)", NamedTextColor.GRAY));
        player.sendMessage(
                Component.text("• Use '/waypoints removetoken' to extract your tokens back", NamedTextColor.GRAY));
        player.sendMessage(Component.text("• Teleport charges are used for waypoint teleports (1 per teleport)",
                NamedTextColor.GRAY));
    }

    /**
     * Converts player facing direction to banner rotation value
     * Banner rotations: 0=South, 4=West, 8=North, 12=East
     */
    private org.bukkit.block.BlockFace getBannerRotationFromPlayer(Player player) {
        float yaw = player.getLocation().getYaw();
        if (yaw < 0)
            yaw += 360;

        // Convert yaw to banner rotation (flipped to match tent orientation)
        if (yaw >= 315 || yaw < 45)
            return org.bukkit.block.BlockFace.NORTH; // 8
        else if (yaw >= 45 && yaw < 135)
            return org.bukkit.block.BlockFace.EAST; // 12
        else if (yaw >= 135 && yaw < 225)
            return org.bukkit.block.BlockFace.SOUTH; // 0
        else
            return org.bukkit.block.BlockFace.WEST; // 4
    }
}
